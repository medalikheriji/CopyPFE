...
import { Frais } from 'src/app/_models/frais';
...
export class ExpensesComponent implements OnInit {
  ...
  */
  ...
  searchField : string ="";
  displayedColumns: string[] = ["nameOfFrais", "typeOfFrais", "descOfFrais","calcUnitOfFrais","categoryOfFrais","actions"];
  dataSource !: MatTableDataSource<any>;
  @ViewChild('paginator') paginator! : MatPaginator;
  @ViewChild(MatSort) mySort! : MatSort ;
  /** */
  constructor(private modalService: NgbModal,private formbuilder:FormBuilder,private _fraisService: ExpensesService , private aRouter : ActivatedRoute , private toast : NgToastService , private mapCustomService : MapCustomService , private sanitizer : DomSanitizer ) {
    this.fraisForm = this.formbuilder.group({
      _id:['000000000000000000000000'],
      nameOfFrais:['',Validators.required],
      typeOfFrais:['',Validators.required],
      descOfFrais:[''],
      calcUnitOfFrais:['',Validators.required],
      quantityOfFrais:[0,Validators.required],
      unitPriceOfFrais:[0,Validators.required],
      dateOfFrais:["",Validators.required],
      categoryOfFrais:["",Validators.required],
      stateOfFrais:["En cours"],
      isRefundable:[false],
      isBillable:[true]
    ...
    this._fraisService.findAllFrais().subscribe(data => {
      console.log(data);
      this.fraisList=data;
  ...
  openSm(content : any ) {
    ...
    this.modalName = "Ajouter une frais";
  ...
  deleteFrais(id : any){
    ...
    // });
    Swal.fire({
      title : 'Voulez-vous vraiment supprimer ce frais ?',
      text : 'Vous ne pourrez pas récupérer ce frais !',
      icon : 'info',
      ...
      if (result.value){
        this._fraisService.deleteFrais(id).subscribe(data=> {
          console.log("Frais has been removed");
        ...
        Swal.fire({
          ...
          text : 'Ce frais à supprimé avec succès',
          icon : 'success',
          confirmButtonColor: "#0dcaf0",
          ...
          Swal.fire({
            ...
            text : 'Votre frais est en sécurité',
            icon : 'question',
  ...
  // }
  /** open the modal  */
  openSmEdit(content : any , frais : Frais ) {
    this.modalService.open(content, { size: 'xl' });
    this.modalName = "Modifier ce frais";
    this.fraisForm.reset({
      _id : frais._id,
      nameOfFrais:frais.nameOfFrais,
      typeOfFrais:frais.typeOfFrais,
      descOfFrais:frais.descOfFrais,
      calcUnitOfFrais:frais.calcUnitOfFrais,
      quantityOfFrais:frais.quantityOfFrais,
      unitPriceOfFrais:frais.unitPriceOfFrais,
      dateOfFrais:frais.dateOfFrais,
      categoryOfFrais:frais.categoryOfFrais,
      stateOfFrais:frais.stateOfFrais,
      isRefundable:frais.isRefundable,
      isBillable:frais.isBillable
    });
  ...
  addFrais(){
   ...
   const addedFrais : Frais = {
    // _id : this.fraisForm.get('_id')?.value,
    nameOfFrais : this.fraisForm.get('nameOfFrais')?.value,
    typeOfFrais : this.fraisForm.get('typeOfFrais')?.value,
    descOfFrais : this.fraisForm.get('descOfFrais')?.value,
    calcUnitOfFrais: this.fraisForm.get('calcUnitOfFrais')?.value,
    quantityOfFrais : this.fraisForm.get('quantityOfFrais')?.value ,
    unitPriceOfFrais : this.fraisForm.get('descOfFrais')?.value,
    dateOfFrais: this.fraisForm.get('dateOfFrais')?.value,
    categoryOfFrais : this.fraisForm.get('categoryOfFrais')?.value,
    stateOfFrais : this.fraisForm.get('stateOfFrais')?.value,
    isRefundable: this.fraisForm.get('isRefundable')?.value,
    isBillable : this.fraisForm.get('isBillable')?.value
    // nameOfFrais:frais.nameOfFrais,
    // typeOfFrais:frais.typeOfFrais,
    // descOfFrais:frais.descOfFrais,
    // calcUnitOfFrais:frais.calcUnitOfFrais,
    // quantityOfFrais:frais.quantityOfFrais,
    // unitPriceOfFrais:frais.unitPriceOfFrais,
    // coastOfFrais:frais.coastOfFrais,
    // dateOfFrais:frais.dateOfFrais,
    // categoryOfFrais:frais.categoryOfFrais,
    // stateOfFrais:frais.stateOfFrais,
    // isRefundable:frais.isRefundable,
  ...
  /** */
  if (this.fraisForm.get('_id')?.value != "000000000000000000000000"){
  /** Edit project */
  
  console.log(this.fraisForm.get('_id')?.value);
  console.log(this.fraisForm.value);
  this._fraisService.editFrais(this.fraisForm.get('_id')?.value , this.fraisForm.value ).subscribe( data => {
  ...
  /** Add project */
  console.log('ADD FRAIS');
  console.log(this.fraisForm.get('_id')?.value);
   this._fraisService.addFrais(addedFrais).subscribe(data => {
    this.fraisForm.reset();
     console.log('Frais added ');
     this.ngOnInit();
